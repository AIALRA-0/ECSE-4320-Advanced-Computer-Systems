...
# - When root or with --fix: automatically install perf/numactl/msr-tools/python dependencies and relax perf sysctl settings
# - When non-root or with --detect-only: detect only, do not modify the system
...
# ====== Record Basic Information (no system modification) ======
...
if [[ $FIX -eq 1 ]]; then
  info "Entering auto-fix mode (root or --fix). Missing dependencies will be installed (excluding MLC)."
  case "$PKG_TOOL" in
    apt)
      pkg_install build-essential gcc make git numactl msr-tools \
                  python3 python3-venv python3-pip ca-certificates \
                  linux-tools-common "linux-headers-$(uname -r)" || true

      # Install perf if missing
      if ! command -v perf >/dev/null 2>&1; then
        info "Installing perf for current kernel: $(uname -r)"
        pkg_install "linux-tools-$(uname -r)" "linux-cloud-tools-$(uname -r)" \
                    linux-tools-generic linux-cloud-tools-generic || warn "perf installation failed"
      fi
      ;;
    dnf|yum)
  ...
  # Python venv + scientific packages
  ...
  info "Detection-only mode: dependencies will not be installed or system modified. To auto-fix, run as root or append --fix"
fi
...
info "Detection complete. Data and logs written to: $OUT_DIR"
if [[ $EXIT_CODE -eq 0 ]]; then
  info "Conclusion: Environment meets requirements. You can directly run experiment scripts (sec2~sec8)."
else
  warn "Conclusion: Some requirements unmet (exit code=$EXIT_CODE). Please resolve before running experiment scripts."
fi
exit $EXIT_CODE
